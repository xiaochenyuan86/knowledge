...
int main() {
	...
	cout << "二维数组行数： " << sizeof(arr) / sizeof(arr[0]) << endl;		// arr[0]是一行占的内存数
	cout << "二维数组列数： " << sizeof(arr[0]) / sizeof(arr[0][0]) << endl;	 //arr[0][0]是一个元素占的内存数
...
[关于二维数组地址的csdn博客](https://blog.csdn.net/Lemon_jay/article/details/83007589)
...
> 总结：函数**定义里**小括号内称为**形参**，函数**调用时**传入的参数称为**实参**
...
>[知乎回答](https://www.zhihu.com/question/443195492)
...
```c++
//冒泡排序函数
void bubbleSort(int * arr, int len)  //int * arr 也可以写为 int arr[],要传入地址
{
	for (int i = 0; i < len - 1; i++)  //i可理解为本次排序是为冒泡出第几个元素
	{
		for (int j = 0; j < len - 1 - i; j++)   //i个元素已定位好，不用再管
		{
			if (arr[j] > arr[j + 1])
			{
				int temp = arr[j];
...
> 总结：当数组名传入到函数作为参数时，被退化为**指向首元素的指针**
...
struct student
{
	...
	int age;      //年龄
	int score;    //分数
}stu3; //结构体变量创建方式3 ，若没有stu3则只进行定义，不创建


int main() {

	//结构体变量创建方式1
	struct student stu1; //struct 关键字可以省略x
...
//冒泡排序
void bubbleSort(hero arr[] , int len)    // 传入的是名字
{
