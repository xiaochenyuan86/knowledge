...
![image-20230507112225489](assets/image-20230507112225489.png)
...
栈区数据：直接int、int*、 double等定义出的数据，默认拷贝操作会重新在栈区开辟新空间（整型、整型的指针都存放在栈区）
...
**不管子类如何实现，父类指针释放时，（若用父类指针指向子类对象，该父类指针）总是无法调用子类的析构代码**（除非父类使用 虚析构/纯虚析构），若父类中有开辟在堆区的数据，就需要手动构造析构函数；若仅有开辟在栈区的，则不用程序员管



解决方式：将**父类中的析构函数**改为**虚析构**或者**纯虚析构**
...
​	2. 如果子类中没有堆区数据，可以不写为虚析构或纯虚析构(编译器实现析构)
...
//电脑类，不是抽象类		
//构造函数接受三个子类对象，用三个父类指针指向（多态），可以传入不同厂商定义的子类，运行时才确定
class Computer
{
public:
...
private:
	...
	CPU * m_cpu; //CPU的零件指针			//多态：父类的指针，指向传入的不同子类对象（运行时才确定）
...
void test01()
{
	...
	computer1->work();
	delete computer1;			//执行电脑的析构，把零件的析构也执行了，因为电脑的析构函数包含了对零件的delete，若不包含，就得在这里写，不整洁
...
{
	ofstream ofs;
	ofs.open("test.txt", ios::out);		//创建一个test.txt函数
...
void test01()
{
	...
	//第一种方式	
	//char buf[1024] = { 0 };
	//while (ifs >> buf)			//一行一行读，每次将一行注入到buf中
	//{
	//	cout << buf << endl;
	//}
	...
	//第二种
	//char buf[1024] = { 0 };
	//while (ifs.getline(buf,sizeof(buf)))	 //通过ifs.getline每次获取文件的一行，存入buf中
	//{										 //ifs.getline第二个参数为每次最多读取多少个字符
	//	cout << buf << endl;
	//}

	//第三种
	//string buf;
	//while (getline(ifs, buf))		//全局函数getline每次将ifs的一行读取到buf中
	//{								//第一个参数为输入流对象，第二个参数为存放的地方
	//	cout << buf << endl;
	//}

	char c;							
	while ((c = ifs.get()) != EOF)	//ifs.get()每次只读取文件的一个字符，存到c中
...
参数解释：字符指针buffer指向内存中一段存储空间（写入的数据存放的位置）。len是读写的字节数
...
void test01()
{
	...
	//2、创建输出流对象				创建一个person.txt文件，不加目录则同项目路径
	ofstream ofs("person.txt", ios::out | ios::binary);		//打开文件两种方式都可以
	...
	//4、写文件				//第一个参数传入要写入文件的数据所在位置，第二个代表共写入的字符数
	ofs.write((const char *)&p, sizeof(p));			//Person类型地址强转为 char* 类型的地址
...
参数解释：字符指针buffer指向内存中一段存储空间（将数据读到哪里）。len是读写的字节数
...
void test01()
{
	ifstream ifs("person.txt", ios::in | ios::binary);		//文件的位置在这里确定
	if (!ifs.is_open())
	{
		cout << "文件打开失败" << endl;
	}
	...
	Person p;
	ifs.read((char *)&p, sizeof(p));		//第一个参数传入p的地址，代表将数据读入到p中
											//第二个参数读入多少字符
